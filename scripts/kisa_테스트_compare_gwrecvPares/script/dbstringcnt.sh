...
rm -f `ls | grep result_st_2015\[0-9\]`
...
while [ -e $logPath/$logDate.log ]
	do
	echo "[String (saved before ${searchDate:0:4}/${searchDate:4:2}/${searchDate:6:2})]  [Count (log date :  ${logDate:0:4}/${logDate:4:2}/${logDate:6:2})]" >> $savePath/result_st_$logDate.log
	...
	while read -r line; do buff1="${line[@]}";
		count=0
		chkcnt=`cat $logPath/cutlist_$logDate.log | grep "$buff1" | awk '{print $1,$2}'`
		if [ -n "$chkcnt" ]
		then
	...
	echo "$temp_arr" >> $savePath/result_st_$logDate.log
