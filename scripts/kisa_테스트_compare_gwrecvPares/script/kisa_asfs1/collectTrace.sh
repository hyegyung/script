...
###----log, msg 개수 차이나는 경우 아래 실행
msgline=`cat msg_$1_$today.txt | wc -l`
logline=`cat log_$1_$today.txt | wc -l`
#echo "msg/line = $msgline , $logline"
if [ $msgline -ne $logline ]
then
echo "here!! mapping shell start!"
	./src/map_files.sh $1 $today
fi
###------------------------------------------
